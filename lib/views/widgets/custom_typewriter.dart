...
class CustomTypewriterAnimatedText extends AnimatedText {
  ...
  static const extraLengthForBlinks = 0;
  ...
  Widget completeText(BuildContext context) => RichText(
        text: TextSpan(
          children: [
            TextSpan(
                text: "${text.toString().split(":").first}: ",
                style: TextStyle(
                  color: Colors.red,
                  fontWeight: FontWeight.bold,
                  fontSize: AppSizes.size14,
                )),
            TextSpan(text: text.toString().split(":").last),
            TextSpan(
              text: cursor,
  ...
  Widget animatedBuilder(BuildContext context, Widget? child) {
    ...
    if (typewriterValue == 0) {
      ...
      visibleString = !textCharacters.take(typewriterValue).toString().contains(":")
          ? ""
          : textCharacters.take(typewriterValue).toString().split(":").last;
      visibleStringName = "${textCharacters.take(typewriterValue).toString().split(":").first}: ";
    }
